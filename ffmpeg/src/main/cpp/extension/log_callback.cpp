...
native_log_callback(int level, const char *message) {
    ...
    // 获取打印回调函数
    napi_ref log_callback_ref = native_ctx_get_log_callback_ref();
    if ( log_callback_ref == nullptr ) return;
    napi_value log_callback_value;
    napi_get_reference_value(env, log_callback_ref, &log_callback_value);
    
    // 将 C 字符串转换为 napi_value
    napi_value msg_value;
    napi_create_string_utf8(env, message, NAPI_AUTO_LENGTH, &msg_value);
    
    // 将 level 转换为 napi_value
    ...
    napi_value params[2] = { level_value, msg_value }; // 参数数组
    napi_call_function(env, global_value, log_callback_value, 2, params, nullptr);
}
