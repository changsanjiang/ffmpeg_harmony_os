...
namespace CoreMedia {
    ...
    FilterGraph::FilterGraph(): filter_graph(nullptr), outputs(nullptr), inputs(nullptr) { }
    ...
    int FilterGraph::addAudioBufferSourceFilter(std::string& name, AVRational time_base, int sample_rate, AVSampleFormat sample_fmt, AVChannelLayout ch_layout) {
        if ( filter_graph == nullptr ) {
    ...
    int FilterGraph::addVideoBufferSourceFilter(std::string& name, AVRational time_base, int width, int height, AVPixelFormat pix_fmt, AVRational sar, AVRational frame_rate) {
        if ( filter_graph == nullptr ) {
            return AVERROR_INVALIDDATA;
        }        
    ...
    int FilterGraph::addAudioBufferSinkFilter(std::string& name, int* _Nullable sample_rates, AVSampleFormat* _Nullable sample_fmts, std::string& channel_layout) {
        if ( instances[name] != nullptr ) {
            return AVERROR(EAGAIN);
        }
    ...
    int FilterGraph::addVideoBufferSinkFilter(std::string& name, AVPixelFormat* _Nullable pix_fmts) {
        if ( instances[name] != nullptr ) {
            return AVERROR(EAGAIN);
        }
    ...
    int FilterGraph::addBufferSourceFilter(std::string& name, AVFilterContext* buffer_ctx) {
    ...
    int FilterGraph::addBufferSinkFilter(std::string& name, AVFilterContext* buffersink_ctx) {
        AVFilterInOut *node = avfilter_inout_alloc();
        if ( node == nullptr ) {
            return AVERROR(ENOMEM);
        }
    ...
    int FilterGraph::parse(std::string& filter_descr) {
    ...
    int FilterGraph::addFrame(std::string& src_name, AVFrame* _Nullable frame, int flags) {
    ...
    int FilterGraph::getFrame(std::string& sink_name, AVFrame* _Nonnull frame) {
    ...
    int FilterGraph::eof(std::string& src_name, int64_t pts, int flags) {
        AVFilterContext *buffer_ctx = instances[src_name];
        if ( buffer_ctx == nullptr ) {
            return AVERROR_FILTER_NOT_FOUND;
        }

        return av_buffersrc_close(buffer_ctx, pts, flags);
    }
