...
void AudioPlayer::onReaderErrorCallback(AudioReader* reader, int ff_err) {
    ...
    if ( ff_err == AVERROR(EIO) || ff_err == AVERROR(ECONNABORTED) ) {
        ...
        recreate_reader_task = TaskScheduler::scheduleTask([&] {
            ...
            // 解码数据需要对齐到当前 fifo 的尾部;
            flags.should_align_pts = true; 
        ...
        client_print_message3("AudioPlayer::onReaderErrorCallback after");
        return;
    }
        ...
        error_occurred = false;
        {
            ...
            // dec pkt
                ...
                ret = audio_decoder->decode(next_pkt, [&](AVFrame* filt_frame) {                     
                    int64_t aligned_pts = audio_fifo->getEndPts() != AV_NOPTS_VALUE ? audio_fifo->getEndPts() : AV_NOPTS_VALUE;
                    int64_t start_pts = filt_frame->pts;
                    if ( flags.should_align_pts && aligned_pts != AV_NOPTS_VALUE ) {
...
void AudioPlayer::recreateReader(int64_t start_time_position_ms) {
    client_print_message3("AAAA: AudioPlayer::recreateReader(%lld)", start_time_position_ms);
    if ( audio_reader ) {
        delete audio_reader;
    }
